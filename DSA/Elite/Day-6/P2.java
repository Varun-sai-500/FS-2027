...
public class P2  {
    public static void main(String[] args) {
        ...
        ArrayList<Integer> restrictedRatings = new ArrayList<>();
        for (int i = 0; i < n; i++) {
            movieRatings[i] = sc.nextInt();
        }
        for (int i = 0; i < m; i++) {
            int a = sc.nextInt();
            restrictedRatings.add(a);
        }
        ...
        while (right < n) {
            ...
            for (int i = left; i <= right; i++) {
                if (hs.contains(movieRatings[i])) break;
                if (!restrictedRatings.contains(movieRatings[i])) {
                    hs.add(movieRatings[i]);
                    sum += movieRatings[i];
                } else {
                    // hs.clear();
                    sum = -1;
                ...
                if (i == right) maxTotal = Math.max(maxTotal, sum);
