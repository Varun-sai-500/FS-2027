...
public class P1 {
    static boolean isValid(String e, String s) {
        int i = 0, j = 0;
        while (i < e.length() && j < s.length()) {
            if (e.charAt(i) != s.charAt(j)) return false;
            char ch = e.charAt(i);
            int eCount = 0;
            ...
            if (eCount < 3) {
                if (eCount != sCount) return false;
            } else {
                if (sCount > eCount) return false;
            }
        }
        return i == e.length() && j == s.length();
    }
